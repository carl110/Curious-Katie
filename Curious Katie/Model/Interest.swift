...
class Interests {
    ...
    static func generateGeneralHobbies() -> [Interests] {
        ...
        let hiking = Interests(name: "hiking", description: "a long, vigorous walk, usually on trails (footpaths), in the countryside", requiredEquipment: "Comfortable shoes")
        let running = Interests(name: "running", description: "A method of terrestrial locomotion allowing rapid movement on foot.", requiredEquipment: "running shoes")
        let horseriding = Interests(name: "horse riding", description: "The skill of riding, driving, steeplechasing or vaulting with horses", requiredEquipment: "Horse with a saddle")
        let abseiling = Interests(name: "abseiling", description: "To descend a rock face or other near-vertical surface by using a doubled rope coiled round the body and fixed at a higher point.", requiredEquipment: "Rope, harnes, helmet and carabiners")
        
        hobbies.append(contentsOf: [football, rugby, snooker, swimming, paintballing, hiking, running, horseriding, abseiling])
